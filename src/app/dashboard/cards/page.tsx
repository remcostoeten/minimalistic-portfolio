...
function useGithubData(login: string) {
  ...
  const totalBranches = (githubData?.user.repositories?.nodes || []).reduce(
    (total, repo) => {
  ...
  const mostActiveRepo = githubData.user.repositories.nodes.reduce(
    (maxRepo, repo) => {
      const repoCommits = repo.defaultBranchRef?.target.history.totalCount || 0

      if (
        repoCommits >
        (maxRepo?.defaultBranchRef?.target.history.totalCount || 0)
      ) {
        return repo
      } else {
        return maxRepo
...
export default function Page(): JSX.Element {
  ...
  const {
    ...
    mostActiveRepo,
    commitsLabels,
  } = useGithubData("remcostoeten")

  const DataCard: React.FC<DashboardProps> = ({
    title,
    icon,
    value,
    subtext,
      ...
      <Shell>
          ...
          heading={
            userName ? (
              ...
              <div style={{ display: "flex", alignItems: "center" }}>
                So <SkeletonBar additionalClasses="w-[200px] ml-2" height={4} />
              </div>
            )
          }
        ...
        <div className="grid gap-4 grid-cols-">
          <DataCard
            ...
            icon={Icons.github}
            subtext={`Commits: ${Number(mostActiveRepo?.defaultBranchRef?.target.history.totalCount) || 0}`}
            loading={loading}
          />
          <DataCard
            title="Total Commits"
        ...
        https://gitlab.com/pleio/beheer/-/issues/13231
      </Shell>
    </>
  )
}
