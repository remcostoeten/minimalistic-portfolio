...
function BettingTable() {
  ...
  const handleMultiplierChange1 = (event: {
    target: { value: React.SetStateAction<number> }
  }) => {
    setMultiplier1(event.target.value)
  }

  const handleMultiplierChange2 = (event: {
    target: { value: React.SetStateAction<number> }
  }) => {
  ...
  return (
    <TooltipProvider>
      <div className="p-6 border-shad border text-white flex flex-col gap-4">
        <p className="border-b border-shad ">
          Dubble down calculator. Two seperate collumns to compare strategies.
        </p>
        <form className="mb-4 flex gap-2 text-left flex-wrap ">
          <LabeledInput
            id="bet1"
            label="Enter your first bet"
            value={bet1}
            onChange={handleInputChange1}
          />
          <LabeledInput
            id="bet2"
            label="Enter your second bet"
            value={bet2}
            onChange={handleInputChange2}
          />
          <LabeledInput
            id="wins"
            label="Enter number of wins"
            value={wins}
            onChange={handleWinsChange}
          />
          <LabeledInput
            id="losses"
            label="Enter number of losses"
            value={losses}
            onChange={handleLossesChange}
          />
          <LabeledInput
            id="multiplier1"
            label="Enter your multiplier for bet 1"
            value={multiplier1}
            onChange={handleMultiplierChange1}
          />
          <LabeledInput
            id="multiplier2"
            label="Enter your multiplier for bet 2"
            value={multiplier2}
            onChange={handleMultiplierChange2}
          />
        </form>
        <Button className="w-fit" onClick={clearAll}>
          Clear all
        </Button>
            ...
            <tr>
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="lowercase">
                      Money you're betting in strategy #1
                    </p>
                  </TooltipContent>
                </Tooltip>{" "}
              </th>
              ...
              >
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="lowercase">
                      Money you're betting in strategy #2
                    </p>
                  </TooltipContent>
                </Tooltip>
              </th>
          ...
          <tbody className="divide-y divide-gray-600">
            {[...Array(20)].map((_, i) => (
              <tr key={i}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-text">
                  {(bet1 * Math.pow(multiplier1, i)).toLocaleString()},-
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-text">
                  €{(bet2 * Math.pow(multiplier2, i)).toLocaleString()},-
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-text">
                  {(wins * Math.pow(2, i)).toLocaleString()},-
                </td>
              </tr>
            ))}
          </tbody>
