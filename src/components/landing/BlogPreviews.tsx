  ...
  return (
    <>
      <section className="flex flex-col gap-y-2 pt-0 pb-28">
        <SectionHeading
          title=" Snippets & Blogs"
          icon={<DocumentScannerOutlined />}
          className="mr-1"
        />
        <SectionSubHeading>
          <p className="dark:text-neutral-400">
            Some snippets, documentation, tools and guides i've created for
            myself for future reference. And who knows someone actually finds
            them usefull.
          </p>
        </SectionSubHeading>
        ...
        <div className="blog-preview">
          {blogPosts.map((post, index) => (
            ...
            >
              <Link
                href={post.url}
                className="geist flex gap-1 justify-between items-center text-sm single"
              >
                <span className="flex gap-2">
                  {Array.isArray(post.label) ? (
                    post.label.map((label, index) => (
              ...
              <div className="flex justify-between items-start py-2 px-2">
                <div>
                  <h3 className="font-bold text-xl mb-2 line-clamp-2 text-white text-[24px]">
                    {post.title}
                  </h3>
        ...
        <div className="mt-10">
          ...
          <Button
            className="mt-8"
            variant="ghost"
            aria-label="Read more on my snippets page"
          >
